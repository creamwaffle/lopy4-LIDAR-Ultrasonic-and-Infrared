...
def reading():
    ...
    apin = adc.channel(pin='P16',attn=adc.ATTN_0DB)   # create an analog pin on P16
    while True:
        val = apin()                    # read an analog value
        time.sleep(0.1)
        a_lock.acquire()
        dist = (val - 6300.2) / -39.703    #ultrasonic input to distance conversion

        #noise rejection
        if (dist > 128 or dist < 0):
            dist = old_dist
        else:
            old_dist = dist
        a_lock.release()
...
def sending():
    ...
    dev_addr = struct.unpack(">l", binascii.unhexlify('9D470B81'))[0] #70B3D5499BF6D450
